...
class ChooseUsernameScreen extends StatefulWidget {
  ...
  const ChooseUsernameScreen({Key? key, required this.email, required this.token}) : super(key: key);
...
class _ChooseUsernameScreenState extends State<ChooseUsernameScreen> with TickerProviderStateMixin {
  ...
  Future<void> onSetUsername() async {
    if (_usernameTextField.isNotEmpty) {
      ...
      int statusCode = await chooseUsername(_usernameTextField, widget.email, widget.token);
